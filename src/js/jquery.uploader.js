    ...
    $.fn.uploader = function(q) {
        return this.each(function(t, r) {
            var s = $(r),
                ...
                l = $(),
                sl = [],
                n_f = $.isFunction(q) ? q(s, $.fn.uploader.defaults) : q,
                n = n_f && $.isPlainObject(n_f) ? $.extend(true, {}, $.fn.uploader.defaults, n_f) : $.fn.uploader.defaults,
                f = {
                    init: function() {
                        s.wrap('<div class="uploader"></div>');
                        f._set('props');
                        s.prop("uploader").boxEl = p = s.closest(b);
                        f._changeInput();
                    },
                    _bindInput: function() {
                        ...
                        if (s.get(0)
                            ...
                            if (n.changeInput) {
                                switch (typeof n.changeInput) {
                                    case "boolean":
                                        o = $('<div class="uploader-input"><div class="uploader-input-caption"><span>' + n.captions.feedback + '</span></div><div class="uploader-input-button">' + n.captions.button + '</div></div>');
                                        break;
                                    case "string":
                                    case "object":
                                        o = $(n.changeInput);
                    ...
                    _thumbCreator: {
                        create: function(i) {
                            var file = f.files[i],
                                ...
                                .pop()
                                .toLowerCase() : "",
                                progressBar = n.uploadFile ? '<div class="uploader-progress">' + n.templates.progressBar + '</div>' : '',
                                opts = {
                                    id: id,
                                    name: name,
                                    size: size,
                        ...
                        renderFile: function(file, html, opts) {
                            ...
                            if (window.File && window.FileList && window.FileReader && opts.type == "image" && opts.size < 1e+7) {
                                ...
                                y.onload = function(e) {
                                    var m = html.find('.uploader-item-thumb-image.fi-loading');
                    ...
                    _remove: function(e, el) {
                        if (el.binded) {
                            if (typeof(el.data.id) != "undefined") {
                                el = l.find(n.templates._selectors.item + "[data-uploader-index='" + el.data.id + "']");
                                if (el.length == 0) {
                                    return false
                                }
                            }
                        ...
                        var excl_input = function(val) {
                                ...
                                if (input.length == 0) {
                                    input = $('<input type="hidden" name="uploader-items-exclude-' + (n.excludeName ? n.excludeName : (s.attr("name")
                        ...
                        var attrId = el.get(0)
                            .uploader_id || el.attr('data-uploader-index'),
                            id = null;
            ...
            s.on("uploader.append", function(e, data) {
                f._append(e, data)
            }).on("uploader.remove", function(e, data) {
                data.binded = true;
                f._remove(e, data);
            }).on("uploader.reset", function(e) {
                f._reset();
                f._clear();
                return true;
